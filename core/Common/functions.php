...
function U($url='',$vars='',$suffix=true,$domain=false,$app_url='') {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        $url        =   $app_url.'?'.C('VAR_MODULE')."={$module}&".http_build_query(array_reverse($var));
        if($urlCase){
            $url    =   strtolower($url);
        }        
        if(!empty($vars)) {
        ...
        if(isset($route)) {
            $url    =   $app_url.'/'.rtrim($url,$depr);
        }else{
            $module =   (defined('BIND_MODULE') && BIND_MODULE==$module )? '' : $module;
            $url    =   $app_url.'/'.($module?$module.MODULE_PATHINFO_DEPR:'').implode($depr,array_reverse($var));
        }
        if($urlCase){
            $url    =   strtolower($url);
        }
