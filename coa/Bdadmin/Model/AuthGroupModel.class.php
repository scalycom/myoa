...
class AuthGroupModel extends Model {
    ...
    public function getGroups($where=array()){
        $map = array('status'=>1,'type'=>self::TYPE_ADMIN,'module'=>'bdadmin');
    ...
    public function addToGroup($uid,$gid){
        ...
        if( $del!==false ){
            foreach ($uid_arr as $u){
            	//判断用户id是否合法
            	if(M('Aduser')->getFieldByUid($u,'ad_id') == false){
    ...
    static public function memberInGroup($group_id){
        ...
        $list     = M() ->field('m.ad_id,u.ad_name,m.login_time,m.login_ip,m.status')
                       ->table($l_table.' m')
                       ->join($r_table.' a ON m.ad_id=a.ad_id')
                       ->join($r_table2.' u ON m.ad_id=u.id')
