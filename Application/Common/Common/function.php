 ...
 *
 */
function list_to_tree($list, $pk = 'cat_id', $pid = 'pid', $child = '_child', $root = 0)
{
    // 创建Tree
    $tree = array();
    if (is_array($list)) {
 ...
 *         yangweijie <yangweijiester@gmail.com>
 */
...
function get_username($uid = 0)
{
    ...
    if (! ($uid && is_numeric($uid))) { // 获取当前登录用户名
        return session('user_auth.ad_name');
    }
    ...
    if (isset($list[$key])) { // 已缓存，直接使用
        ...
        $info = M('Aduser')->where(array(
            'ad_id' => $uid
        ))->find();
        if ($info && isset($info['ad_name'])) {
            $name = $list[$key] = $info['ad_name'];
            /* 缓存用户 */
            $count = count($list);
...
function get_ad_name($uid = 0)
{
    ...
    if (isset($list[$key])) { // 已缓存，直接使用
        ...
        $info = M('Aduser')->field('ad_name')->find($uid);
        if ($info !== false && $info['ad_name']) {
            $ad_name = $info['ad_name'];
            $name = $list[$key] = $ad_name;
            /* 缓存用户 */
            $count = count($list);
            $max = C('USER_MAX_CACHE');
            while ($count -- > $max) {
 ...
 *            日志标识
 */
function addWeixinLog($data, $data_post = '', $wechat = false, $log_type = '')
{
    if (C('WX_LOG_START') == 1) {
        $log['cTime'] = time();
 ...
 * @param unknown $pass            
 */
function user_pass_encrypt($pass)
{
    return md5(md5($pass) . 'cjadmin');
}
 ...
 *            是否加上域名
 */
function url_to_http($url = '', $server = 1)
{
    if (! is_http($url)) {
 ...
 */
function replace_str($str, $find, $replace_to = '')
{
    if (is_array($find)) {
        foreach ($find as $k => $v) {
...
function get_region()
{
    $region = S('Region_list');
    if (! $region) {
        $regionlist = M('Region')->select();
        foreach ($regionlist as $k => $v) {
            $region[$v['region_id']] = $v;
        }
        
        S('Region_list', $region);
    }
 ...
 *            0国家 1省 2市 3县
 */
function get_region_type($type = '')
{
    $regionlist = get_region();
    foreach ($regionlist as $k => $v) {
        $region[$v['region_type']][$k] = $v;
    }
 ...
 * @param number $pid            
 */
function get_child_region($pid = 0)
{
    $regionlist = get_region();
    foreach ($regionlist as $k => $v) {
        $region[$v['parent_id']][$k] = $v;
    }
    
    return $region[$pid];
}

function add_pay_log($result)
{
    $data['attach'] = $result['attach'];
    $data['bank_type'] = $result['bank_type'];
    $data['pay_fee'] = floatval($result['total_fee'])/100;
    ...
    $data['pay_info'] = $result['pay_info'];
    $data['uid'] = intval($result['uid']);
...
//获取分销商佣金比例
function get_agent_yongjin(){
    $agent_yongjin = S('AGENT_YONGJIN');
    if(!$agent_yongjin){
        $agent_rank  = M('Agentrank')->select();
        foreach ($agent_rank as $k=>$v){
            $agent[$v['rank_id']] = $v;
            $agent[$v['first_money']] = $v['first_ratio'];
            $agent[$v['second_money']] = $v['second_ratio'];
            $agent[$v['third_money']] = $v['third_ratio'];
        }
        
        $rank0 = M('Agent_set')->where('id=1')->find();
        $agent[0] = $rank0;
        
        $agent_yongjin = $agent;
        S('AGENT_YONGJIN',$agent_yongjin);
    }
    return $agent_yongjin;
}

function add_account_log($uid,$account,$type=1,$info='账户资金调整'){
    $data['uid'] = intval($uid);
    $data['change_type'] = intval($type);
    if($data['change_type'] == 1){
        $data['user_money'] = floatval($account);
    }else{
        $data['user_jifen'] = intval($account);
    }
...
function add_yongjin_log($uid,$account,$type=1,$info='账户资金调整'){
    $data['uid'] = intval($uid);
    $data['change_type'] = intval($type);
    if($data['change_type'] == 1){
        $data['user_money'] = floatval($account);
